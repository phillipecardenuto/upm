...
Authors: João Phillipe Cardenuto, Daniel Moreira.
Created: 2021/09/01
RECOD.ai
"""
...
from ip.image_descriptor import sift_detect_rsift_describe
...
def vlfeat_sift(input_image,
    ...
    # To use sift:
    """
    input_image must be ``float32`` and grayscale (either a single channel as 
    the last axis, or no channel).
    """
    ...
    vis = np.array(Image.open(input_image))
    for point in locations:
        x,y = point
        vis = cv2.circle(vis,(x,y) , radius=3, color=(0, 0, 255), thickness=-1)
    Image.fromarray(vis).save('test_without_h.png')
...
def vlfeat_dsift(input_image,
    ...
    print(locations.shape)
    vis = np.array(Image.open(input_image))
    #locations = locations[locations[:,1] <10]
    for point in locations:
        x,y = point
        vis = cv2.circle(vis,(x,y) , radius=3, color=(0, 0, 255), thickness=-1)
    Image.fromarray(vis).save('test_dsift.png')
    print(descriptors.shape)
    """
...
def vlfeat_phow(input_image, flip = False):
    ...
    for point in locations:
        x,y = point
        vis = cv2.circle(vis,(x,y) , radius=3, color=(0, 0, 255), thickness=-1)
    Image.fromarray(vis).save('test_phow.png')
    print(descriptors.shape)
    """
...
def _increase_image_if_necessary(image, mask=None, min_image_size=10000):
    """
    Keeps doubling the size of the given <image> and respective <mask> up to the
    point of having more pixels than <min_image_size>.

    Returns the re-sized image and mask, and the number of times they were 
    re-sized.
    """
    resize_count = 0
...
def cv_sift(input_image,
    ...
    Parameters:
        input_image: path to image.
        kp_count: number of keypoints to be detected.
        hist_equalization: whether to apply histogram equalization or not.
        contrastThreshold: contrast threshold for keypoint detection.
        sigma: sigma for keypoint detection.
        eps: epsilon for normalization.
        root_sift: whether to apply root sift or not.
        flip: whether to flip the image or not.
    ...
    for point in keypoints:
        x,y = point
        #x , y = int(np.round(x,1)), int(np.round(y,1))
        vis = cv2.circle(vis,(x,y) , radius=3, color=(0, 255, 0), thickness=-1)
    Image.fromarray(vis).save('test_sift_with_h.png')

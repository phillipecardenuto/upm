...
def panel_evaluation(predicted_groups, annotated_groups, panel_dataset, predicted_adj_matrix, save_file):
    """
    ...
    if len(rp) ==0:
        ...
        results['relationship_quality_per_class'] = {}
        for cls in ['Blots', 'Flow Cytometry', 'Microscopy']:
            results['relationship_quality_per_class'][cls] = 0
        
        
    else:
        results['relationship_precision'] = rp[rp['Class'] != 'Graphs']['Precision'].mean() # Remove Graphs from eval
        results['relationship_recall'] = rr[rr['Class'] != 'Graphs']['Recall'].mean() # Remove Graphs from eval
        results['relationship_quality'] = f1(results['relationship_precision'], results['relationship_recall'])

        results['relationship_quality_per_class'] = {}
        rp_by_class = rp.groupby("Class")['Precision'].mean().to_dict()
        rr_by_class = rr.groupby("Class")['Recall'].mean().to_dict()

        # get all classes
        all_classes = set(rp_by_class).union(set(rr_by_class))
        for cls in all_classes:
            p = rp_by_class[cls] if rp_by_class.get(cls) else 0
            r = rr_by_class[cls] if rr_by_class.get(cls) else 0
            results['relationship_quality_per_class'][cls] = f1(p, r)
    ...
    if len(gp) ==0:
        ...
        results['grouping_quality_per_class'] = {}
        for cls in ['Blots', 'Flow Cytometry', 'Microscopy']:
            results['grouping_quality_per_class'][cls] = 0
        
    else:
        results['grouping_precision'] = gp[gp['Class'] != 'Graphs']['Precision'].mean() # Remove Graphs from eval
        results['grouping_recall'] = gr[gr['Class'] != 'Graphs']['Recall'].mean() # Remove Graphs from eval
        ...
        for cls in all_classes:
            p = rp_by_class[cls] if rp_by_class.get(cls) else 0
            r = rr_by_class[cls] if rr_by_class.get(cls) else 0
            results['grouping_quality_per_class'][cls] = f1(p, r)

    # Global Eval
    if len(gp) == 0  or len(rp) ==0:
        results['global_precision'] = 0
        results['global_recall'] = 0
        results['global_quality'] = 0
    else:
        results['global_precision'] = global_precision(predicted_groups, annotated_groups, ignore_graphs=True)
        results['global_recall'] = global_recall(predicted_groups, annotated_groups, ignore_graphs=True)
...
def doc_evaluation(predicted_groups, annotated_groups, doc_dataset, predicted_adj_matrix, save_file):
    """
    Perform panel evaluation and dump the eval into a JSON file
    """

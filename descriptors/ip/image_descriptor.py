...
def sift_detect_rsift_describe(image, kp_count=2000, mask=None, mask_text=True, mask_background=True, eps=1e-7):
    ...
    sift_detector = cv2.SIFT_create(nfeatures=kp_count, contrastThreshold=0.0, sigma=3.2)
...
def describe(image, kp_count=2000):
    ...
    return regl_keypoints, regl_descriptions, flip_keypoints, flip_descriptions
...
def store_descriptions(keypoints, descriptions, description_file_path):
    ...
    # print('Saved', description_file_path)
...
def load_descriptions(description_file_path):
    ...
    content = numpy.load(description_file_path, allow_pickle=True)
    for c in content:
        x = c[0]
        y = c[1]
        size = c[2]

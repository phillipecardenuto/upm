...
def panel_evaluation(predicted_groups, annotated_groups, panel_dataset, predicted_adj_matrix, xclass=["Graphs"]):
    """
    Perform panel evaluation and dump the eval into a JSON file

    xclass: exclude class from eval
    """
    ...
    if len(rp) ==0:
        ...
        # remove panels from xclass from counting
        results['relationship_precision'] = rp[~rp['Class'].isin(xclass)]['Precision'].mean() # Remove Graphs from eval
        results['relationship_recall'] = rr[~rr['Class'].isin(xclass)]['Recall'].mean() # Remove Graphs from eval
    ...
    if len(gp) ==0:
        ...
        results['grouping_precision'] = gp[~gp['Class'].isin(xclass)]['Precision'].mean() # Remove Graphs from eval
        results['grouping_recall'] = gr[~gr['Class'].isin(xclass)]['Recall'].mean() # Remove Graphs from eval
